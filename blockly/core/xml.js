...
Blockly.Xml.blockToDom = function (block, opt_noId) {
    ...
    if (block.getConFromId()) {
        var confromElement = goog.dom.createDom('fromid', null, block.getConFromId());
        element.appendChild(confromElement);
    }

    if (block.getConToId()) {
        var contoElement = goog.dom.createDom('toid', null, block.getConToId());
        element.appendChild(contoElement);
    }
    ...
    for (var i = 0, xmlChild; xmlChild = xmlBlock.childNodes[i]; i++) {
        ...
        switch (xmlChild.nodeName.toLowerCase()) {
            ...
            case 'data':
                ...
                break;
            case 'fromid':
                block.bindConFrom(xmlChild.textContent);
                break;
            case 'toid':
                block.bindConTo(xmlChild.textContent);
                break;
            case 'title':
...
/**
 * Remove any block with confrom od conto.
 * @param {!Element} xmlBlock XML block element.
 */
Blockly.Xml.deleteConBlocks = function (xmlBlock) {
    for (var i = xmlBlock.childNodes.length - 1; i >= 0; i--) {
        let child = xmlBlock.childNodes[i];
        this.deleteConChild_(child);
    }
};

/**
 * Remove child blocks with fromid or toid
 * @param {!Element} child XML child element.
 * @private
 */
Blockly.Xml.deleteConChild_ = function (children) {
    for (var i = 0; i < children.childNodes.length; i++) {
        let child = children.childNodes[i];
