...
Blockly.Block = function (workspace, prototypeName, opt_id) {
    ...
    /** 
      * The LTR workspace with RTL blocks option
      * @type {boolean} */
    this.RTLMod = workspace.RTLMod;

    /** 
     * Line object - the graphical part of connected blocks
     * @type {!Blockly.Line}
     * @private
     */
    this.line_ = null;
    ...
    /**
     * If true, the button (color field) for making connections is enabled
     * @type {bool}
     */
    this.canCreateCons = false;

    /**
     * If true, a button (color field) for making a connection is visible
     * @type {bool}
     * @private
     */
    this.createConsVisible_ = false;

    /**
     * The id of a block that is connected to this block
     * @type {string}
     * @private
     */
    this.conToId_ = null;
    ...
    /**
     * The id of a block that this block is connected to
     * @type {string}
     * @private
     */
    this.conFromId_ = null;
...
/**
 * Sets property of a block - if this block can create connection blocks
 * @param {boolean} canCreateCons 
 */
Blockly.Block.prototype.setCanCreateCons = function (canCreateCons) {
    this.canCreateCons = canCreateCons;
};

/**
 * Creates the connection blocks on this block 
 * and connects them together
 */
Blockly.Block.prototype.createCons = function () {

    var spacing = this.workspace.getGrid().getSpacing();
    ...
    var fromBlock = this.workspace.newBlock('node_confrom');
    fromBlock.initSvg();
    //if (!isNaN(xy.x) && !isNaN(xy.y)) {
    fromBlock.moveBy(xy.x + 0 * spacing, xy.y);
    //}
    fromBlock.render(false);

    var toBlock = this.workspace.newBlock('node_conto');
    toBlock.initSvg();
    //if (!isNaN(xy.x) && !isNaN(xy.y)) {
    toBlock.moveBy(xy.x + 1 * spacing, xy.y);
    //}
    toBlock.render(false);

    fromBlock.link_(toBlock);

    Blockly.Events.enable();
    Blockly.Events.setGroup(true);

    Blockly.Events.fire(new Blockly.Events.BlockCreate(fromBlock));
    fromBlock.inputList[0].connection.connect(this.outputConnection);
    this.disableCreateCons(true);
    this.snapToGrid();
    Blockly.Events.fire(new Blockly.Events.BlockCreate(toBlock));
 ...
 * @param {bool} ofThisBlock return colour of this block - no neighbours
 * @return {string} #RRGGBB string.
 */
Blockly.Block.prototype.getColour = function (ofThisBlock) {
    ...
    if (child && this.getConToId()) {
        colour = child.getColour(true);
    }

    else if (parent && this.getConFromId()) {
        colour = parent.getColour(true);
    }

    else if (!parent && !child && (this.getConToId() || this.getConFromId())) {
        colour = Blockly.Constants.NodeBlockly.Colour.Connection;
    }
...
Blockly.Block.prototype.getInheritedDisabled = function () {
    ...
    if (this.getConTo()) {
        ancestor = this.getConTo();
    } else {
        ancestor = this.getSurroundParent();
    }

    if (ancestor) {
        var start_id = ancestor.id;
        ...
        while (ancestor) {
            ...
            if (ancestor.getConTo()) {
                ancestor = ancestor.getConTo();
            } else {
                ancestor = ancestor.getSurroundParent();
            }

            if (ancestor && ancestor.id == start_id) {
                console.warn("Backlooping blocks.");
                ancestor = null;
            }
        }
    }
...
/**
 * If the connection from this block lead to block, then 
 * this block is backlooping
 * @return {boolean} True backlooping.
 */
Blockly.Block.prototype.isBacklooping = function () {
...
/**
 * Returns the block that this block is connected to
 * @return {Blockly.Block} ConnectedTo block
 */
Blockly.Block.prototype.getConTo = function () {
    if (!this.conToId_)
        return null;
    else if (this.workspace) {
        return this.workspace.getBlockById(this.conToId_);
        //return this.workspace.blockDB_[this.conToId_];
    } else {
        return null;
    }
};
...
/**
 * Returns the block that this block is connected from
 * @return {Blockly.Block} ConnectedFrom block
 */
Blockly.Block.prototype.getConFrom = function () {
    if (!this.conFromId_)
        return null;
    else if (this.workspace) {
        return this.workspace.getBlockById(this.conFromId_);
        //return this.workspace.blockDB_[this.conFromId_];
    } else {
        return null;
    }
};
...
/**
 * Returns id of a block that is connected to this block
 * @return {bool}
 */
Blockly.Block.prototype.getConToId = function () {
    return this.conToId_;
};

/**
 * Returns id of a block that this block is connected to
 * @return {bool} 
 */
Blockly.Block.prototype.getConFromId = function () {
    return this.conFromId_;
};

/**
 * Binds a connectedTo block to this block
 * @param {string} conToId The ID of a block that this block is connected to
 */
Blockly.Block.prototype.bindConTo = function (conToId) {
    this.conToId_ = conToId;
};

/**
 * Binds a connectedFrom block to this block
 * @param {string} conToId The ID of a block that this block is connected from
 */
Blockly.Block.prototype.bindConFrom = function (conFromId) {
    this.conFromId_ = conFromId;
};
...
/**
 * Connects this block to another - creating the connection blocks
 * @param {!Blockly.Block} connectingTo
 * @private
 */
Blockly.Block.prototype.link_ = function (block) {
    this.bindConTo(block.id);
    block.bindConFrom(this.id);  
};


/**
 * Updates the line graphics of this and every descentant block
 * @param {!goog.math.Coordinate} base_xy
 * @param {!goog.math.Coordinate} tip_xy
 */
Blockly.Block.prototype.updateAllLines = function (base_xy, tip_xy) {

    var descendants = this.getDescendants(false);
    for (var i = 0, descendant; descendant = descendants[i]; i++) {
        descendant.updateLine();
    }
}

/**
 * Updates the line graphics of this block
 * @param {!goog.math.Coordinate} base_xy
 * @param {!goog.math.Coordinate} tip_xy
 */
Blockly.Block.prototype.updateLine = function (base_xy, tip_xy) {

    var contoblock = null;
    var confromblock = null;

    if (!this.getConToId() && !this.getConFromId()) {
        return;
    }

    if (this.getConToId()) {
        contoblock = this.getConTo();
        if (!contoblock) {
            return;
        }
        confromblock = this;
    }
    else if (this.getConFromId()) {
        contoblock = this;
        confromblock = this.getConFrom();
        if (!confromblock) {
            return;
        }
    }

    if (!confromblock.line_) {
        confromblock.line_ = new Blockly.Line(confromblock.workspace, contoblock.getRelativeToSurfaceXY());
    }

    if (!base_xy) {
        base_xy = confromblock.getRelativeToSurfaceXY();
        base_xy.x -= 7;
        base_xy.y += confromblock.height / 2;
    }

    if (!tip_xy) {
        tip_xy = contoblock.getRelativeToSurfaceXY();
        tip_xy.x -= 25;
        tip_xy.y += contoblock.height / 2;
    }

    confromblock.line_.setAnchorLocation(base_xy);
    confromblock.line_.setTipLocation(tip_xy);
