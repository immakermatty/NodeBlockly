...
Blockly.Workspace = function (opt_options) {
    ...
    /** @type {boolean} */
    this.RTLMod = !!this.options.RTLMod;
    /** @type {boolean} */
    ...
    /*SW-DT Workspace behaviour modifications*/

    //this.addChangeListener(e => { console.log(e) });

    /**
     * Connection creation field disabling and enabling
     */
    this.addChangeListener(e => {
        ...
        if (e.type == Blockly.Events.BLOCK_MOVE && !this.isDragging()) {
            let block_ = this.getBlockById(e.blockId);
            if (block_ && block_.canCreateCons) {
                block = block_;
                if (block_.getParent()) {
                    disable = true;
                }
        ...
        else if (e.type == Blockly.Events.BLOCK_CREATE && !this.isDragging()) {
            let block_ = this.getBlockById(e.blockId);
            if (block_ && block_.canCreateCons && !block_.getParent()) {
                block = block_;
                enable = true;
            }
            else {
        ...
        if (block && !block.isInFlyout) {
            ...
            if (disable) {              
                block.disableCreateCons(true);
            }

            if (enable) {
                block.enableCreateCons(true);
            }
