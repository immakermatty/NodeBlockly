...
Blockly.BlockSvg.prototype.showContextMenu_ = function (e) {
    ...
    Blockly.ContextMenu.show(e, menuOptions, this.RTLMod ? false : this.RTL);
...
/**
 * Enable the create connections color field (button)
 * @param {boolean} render Re-render block. Defaults to true
 */
Blockly.BlockSvg.prototype.enableCreateCons = function (render, move) {

    if (render === null) {
        render = true;
    }

    if (!this.createConsVisible_) {

        let color = this.getColour();

        let r = parseInt(color.slice(1, 3), 16);
        let g = parseInt(color.slice(3, 5), 16);
        let b = parseInt(color.slice(5, 7), 16);

        color = rgbToHex(r + 55, g + 55, b + 55);

        this.inputList[0].insertFieldAt(0, new Blockly.FieldColour(color), Blockly.Constants.NodeBlockly.CreateConFieldName);

        this.createConsVisible_ = true;
        if (render) {
            this.render();
        }
        ...
        var dx = 1 * this.workspace.getGrid().getSpacing();
        var dy = 0;
...
/**
 * Disable the create connections color field (button)
 * @param {boolean} render Re-render block. Defaults to true
 */
Blockly.BlockSvg.prototype.disableCreateCons = function (render) {
    ...
    if (this.createConsVisible_) {
        for (var i = 0, input; input = this.inputList[i]; i++) {
            for (var j = 0, field; field = input.fieldRow[j]; j++) {
                if (field.name === Blockly.Constants.NodeBlockly.CreateConFieldName) {
                    field.dispose();
                    input.fieldRow.shift();
...
Blockly.BlockSvg.prototype.dispose = function (healStack, animate) {
    ...
    //Removing connection blocks
    if (this.getConToId()) {
        let disabled = Blockly.Events.getDisabled();
        try {
            Blockly.Events.setDisabled(0);
            var block = blockWorkspace.getBlockById(this.getConToId());
            if (block) {
                //console.log(block);
                block.unplug(false);
                block.dispose(healStack, animate);
            }
        } finally {
            Blockly.Events.setDisabled(disabled);
        }
    }

    if (this.getConFromId()) {
        let disabled = Blockly.Events.getDisabled();
        try {
            Blockly.Events.setDisabled(0);
            var block = blockWorkspace.getBlockById(this.getConFromId());
            if (block) {
                //console.log(block);
                var des = block.getDescendants(true);
                if (des.length > 1) {
                    des[1].unplug(false);
                    //Blockly.Events.fire(new Blockly.Events.BlockMove(des[1]));                    
                }
                block.dispose(healStack, animate);
            }
        } finally {
 ...
 * @param {bool} dontRerender Do not bump or rerender
 */
Blockly.BlockSvg.prototype.updateColour = function (dontRerender) {
    ...
    if (!this.workspace.isDragging() && !dontRerender) {
...
Blockly.BlockSvg.prototype.updateDisabled = function () {
    ...
    if (this.workspace.backloop) {
        throw "For this option to work edit code below, so that it not " +
        "go to an infinite loop :-)"
    }

    if (this.isBacklooping()) {
        Blockly.selected.render();
        if (Blockly.selected.getParent()) {
            Blockly.selected.unplug();
        } else {
            Blockly.selected.getChildren()[0].unplug();
        }
    }

    //Also getInheritedDisabled() has backlooping issues
    //I just can't think of a good fix to the infinite loop problem
    if (this.disabled || this.getInheritedDisabled()) {
